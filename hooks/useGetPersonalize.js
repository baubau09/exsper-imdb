...
const useGetPersonalize = (movieid) => {
    ...
    const { data: similarItemData, error: similarItemError } = useSWR(movieid ? `https://z1hi6ssas5.execute-api.ap-southeast-1.amazonaws.com/getSims?itemId=${movieid}&numResults=5` : null, fetcher)
    const { data: reRankingData, error: reRankingError } = useSWR(user ? `https://z1hi6ssas5.execute-api.ap-southeast-1.amazonaws.com/getReranking?userId=${userid}&itemId=${movieid}&numResults=5` : null, fetcher)
